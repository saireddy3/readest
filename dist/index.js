...
var init_misc = __esm({
  "src/utils/misc.ts"() {
    ...
    randomMd5 = () => jsMd5.md5(Math.random().toString());
    getContentMd5 = (content) => jsMd5.md5(JSON.stringify(content));
...
function md5Fingerprint(value) {
  return jsMd5.md5(value).slice(0, 7);
}
async function partialMD5(file) {
  const step = 1024;
  const size = 1024;
  const hasher = jsMd5.md5.create();
  for (let i = -1; i <= 10; i++) {
...
function hexToOklch(hexColor) {
  const { r, g, b } = tinycolor2__default.default(hexColor).toRgb();
...
var getContrastOklch = (hexColor) => {
  return tinycolor2__default.default(hexColor).isDark() ? "100% 0 0" : "0% 0 0";
};
...
var generateLightPalette = ({ bg, fg, primary }) => {
  return {
    "base-100": bg,
    // Main background
    "base-200": tinycolor2__default.default(bg).darken(5).toHexString(),
    // Slightly darker
    "base-300": tinycolor2__default.default(bg).darken(12).toHexString(),
    // More darker
    "base-content": fg,
    // Main text color
    neutral: tinycolor2__default.default(bg).darken(15).desaturate(20).toHexString(),
    // Muted neutral
    "neutral-content": tinycolor2__default.default(fg).lighten(20).desaturate(20).toHexString(),
    // Slightly lighter text
    primary,
    secondary: tinycolor2__default.default(primary).lighten(20).toHexString(),
    // Lighter secondary
    accent: tinycolor2__default.default(primary).analogous()[1].toHexString()
    // Analogous accent
  };
};
var generateDarkPalette = ({ bg, fg, primary }) => {
  return {
    "base-100": bg,
    // Main background
    "base-200": tinycolor2__default.default(bg).lighten(5).toHexString(),
    // Slightly lighter
    "base-300": tinycolor2__default.default(bg).lighten(12).toHexString(),
    // More lighter
    "base-content": fg,
    // Main text color
    neutral: tinycolor2__default.default(bg).lighten(15).desaturate(20).toHexString(),
    // Muted neutral
    "neutral-content": tinycolor2__default.default(fg).darken(20).desaturate(20).toHexString(),
    // Darkened text
    primary,
    secondary: tinycolor2__default.default(primary).darken(20).toHexString(),
    // Darker secondary
    accent: tinycolor2__default.default(primary).triad()[1].toHexString()
    // Triad accent
  };
};
var themes = [
  ...
  React42.useEffect(() => {
    ...
    const initSettings = async () => {
      try {
        ...
        const hash = jsMd5.md5(arrayBuffer);
...
exports.wrappedFoliateView = wrappedFoliateView;
