...
var CFI4 = require('@skillsoft/foliate-js/epubcfi.js');
var clsx8 = require('clsx');
var React42 = require('react');
var PropTypes13 = require('prop-types');
var zustand = require('zustand');
var tinycolor = require('tinycolor2');
var i18n = require('i18next');
var reactI18next = require('react-i18next');
var HttpApi = require('i18next-http-backend');
...
var overlayer_js = require('@skillsoft/foliate-js/overlayer.js');
var footnotes_js = require('@skillsoft/foliate-js/footnotes.js');
...
var PropTypes13__default = /*#__PURE__*/_interopDefault(PropTypes13);
...
var cssbeautify__default = /*#__PURE__*/_interopDefault(cssbeautify);
...
// src/utils/misc.js
var stubTranslation, getUserLocale, getUserLang, isCJKEnv, isValidURL, isContentURI, uniqueId, makeSafeFilename, getContentMd5, randomMd5, getOSPlatform;
var init_misc = __esm({
  "src/utils/misc.js"() {
    ...
    isContentURI = (uri) => {
      return uri == null ? void 0 : uri.startsWith("content://");
    };
    uniqueId = () => {
...
// src/utils/rtl.js
var isRTL, getDirFromLanguage, getDirFromUILanguage;
var init_rtl = __esm({
  "src/utils/rtl.js"() {
    isRTL = (langCode) => {
...
var init_book = __esm({
  "src/utils/book.js"() {
    ...
    getBaseFilename = (filename) => {
      ...
      const baseName = ((_a = normalizedPath.split("/").pop()) == null ? void 0 : _a.split(".").slice(0, -1).join(".")) || "";
      return baseName;
    };
    INIT_BOOK_CONFIG = {
      updatedAt: 0
    ...
    getBookLangCode = (lang) => {
      try {
        const bookLang = typeof lang === "string" ? lang : lang == null ? void 0 : lang[0];
    ...
    formatAuthors = (contributors, bookLang) => {
      ...
      return Array.isArray(contributors) ? listFormater(langCode === "zh", langCode).format(
        contributors.map(
          (contributor) => typeof contributor === "string" ? contributor : formatLanguageMap(contributor == null ? void 0 : contributor.name)
        )
      ) : typeof contributors === "string" ? contributors : formatLanguageMap(contributors == null ? void 0 : contributors.name);
    };
    formatTitle = (title) => {
...
var init_file = __esm({
  "src/utils/file.js"() {
    ...
    DeferredBlob = class _DeferredBlob extends Blob {
      /**
       * @param {Promise<ArrayBuffer>} promise - Promise that resolves to the blob's content
      ...
      /**
       * @returns {Promise<ArrayBuffer>} The blob's content
       */
      async arrayBuffer() {
        return this.promise;
      }
       ...
       * @returns {Blob} A new blob containing the specified range
       */
    ...
    RemoteFile = class _RemoteFile extends File {
      ...
      /** @private */
      #type = "";
      /** @private @type {Map<number, ArrayBuffer>} */
      ...
      async _open_with_range() {
        ...
        this.#size = Number((_a = response.headers.get("content-range")) == null ? void 0 : _a.split("/")[1]);
        this.#type = response.headers.get("content-type") || "";
      }
      /**
       * @returns {Promise<void>}
      ...
      /**
       * @private
       * @param {number} chunkStart - Chunk start position
       */
       ...
       * @param {number} [end=this.size] - End position
       * @param {string} [contentType=this.type] - Content type
       * @returns {Blob}
       */
...
var init_serializer = __esm({
  "src/utils/serializer.js"() {
    serializeConfig = (config2, globalViewSettings, defaultSearchConfig) => {
      config2 = JSON.parse(JSON.stringify(config2));
      const viewSettings = config2.viewSettings;
      const searchConfig = config2.searchConfig;
      config2.viewSettings = Object.entries(viewSettings).reduce(
        (acc, [key, value]) => {
      ...
      config2.searchConfig = Object.entries(searchConfig).reduce(
        (acc, [key, value]) => {
    ...
    deserializeConfig = (str, globalViewSettings, defaultSearchConfig) => {
      const config2 = JSON.parse(str);
      const { viewSettings, searchConfig } = config2;
      config2.viewSettings = { ...globalViewSettings, ...viewSettings };
      config2.searchConfig = { ...defaultSearchConfig, ...searchConfig };
...
var init_storage = __esm({
  "src/libs/storage.js"() {
    ...
    downloadFile = async (filePath, onProgress) => {
...
var init_txt = __esm({
  "src/utils/txt.js"() {
    ...
    TxtToEpubConverter = class {
       ...
       * @param {Txt2EpubOptions} options - Conversion options
       ...
       * @param {string} txtContent - Text content
       * @param {Metadata} metadata - Book metadata
       * @param {ExtractChapterOptions} option - Extraction options
       * @returns {Chapter[]} Extracted chapters
       * @private
       */
...
`);
        ...
        for (const segment of segments) {
          ...
          for (let j = 1; j < matches.length; j += 2) {
            const title = ((_a = matches[j]) == null ? void 0 : _a.trim()) || "";
            const content = ((_b = matches[j + 1]) == null ? void 0 : _b.trim()) || "";
            let isVolume = false;
            if (language === "zh") {
       ...
       * @param {Chapter[]} chapters - Book chapters
       * @param {Metadata} metadata - Book metadata
    ...
    `;
       ...
       * @param {ArrayBuffer} buffer - Text buffer
       * @returns {string|undefined} Detected encoding
       * @private
       */
      ...
      /**
       * Detect text language from header
       * @param {string} fileHeader - File header text
...
var init_appService = __esm({
  "src/services/appService.js"() {
    ...
    BaseAppService = class {
       ...
       * @returns {Promise<void>}
       */
      async saveBookConfig(book, config2, settings) {
        var _a, _b, _c;
        try {
          console.log(`\u{1F4DD} Saving book config for ${book.hash}`, {
            hasBooknotes: ((_a = config2.booknotes) == null ? void 0 : _a.length) || 0,
            progressPosition: ((_b = config2.progress) == null ? void 0 : _b[0]) || 0,
            location: ((_c = config2.location) == null ? void 0 : _c.substring(0, 30)) || "none"
          });
          let serializedConfig;
          if (settings) {
            const { globalViewSettings } = settings;
            serializedConfig = serializeConfig(config2, globalViewSettings, DEFAULT_BOOK_SEARCH_CONFIG);
          } else {
            serializedConfig = JSON.stringify(config2);
          }
...
var isWebAppPlatform, isPWA, EnvConfig, environment_default;
    ...
    EnvConfig = class {
      ...
      async getAppService() {
        ...
        if (this.isWebApp) {
          ...
          this.appService = new WebAppService2();
        } else {
          const { AppService } = await Promise.resolve().then(() => (init_appService(), appService_exports));
          this.appService = new AppService();
        }
        return this.appService;
      }
    };
    environment_default = new EnvConfig();
  }
});
...
// src/app/reader/components/Reader.jsx
init_md5();
...
EnvProvider.propTypes = {
  children: PropTypes13__default.default.node.isRequired
};
var useEnv = () => {
...
var getContrastOklch = (color1, color2) => {
...
var generateLightPalette = ({ bg, fg, primary }) => {
  return {
    "base-100": bg,
    // Main background
    "base-200": tinycolor__default.default(bg).darken(5).toHexString(),
    // Slightly darker
    "base-300": tinycolor__default.default(bg).darken(12).toHexString(),
    // More darker
    "base-content": fg,
    // Main text color
    neutral: tinycolor__default.default(bg).darken(15).desaturate(20).toHexString(),
    // Muted neutral
    "neutral-content": tinycolor__default.default(fg).lighten(20).desaturate(20).toHexString(),
    // Slightly lighter text
    primary,
    secondary: tinycolor__default.default(primary).lighten(20).toHexString(),
    // Lighter secondary
    accent: tinycolor__default.default(primary).analogous()[1].toHexString()
    // Analogous accent
  };
};
var generateDarkPalette = ({ bg, fg, primary }) => {
  return {
    "base-100": bg,
    // Main background
    "base-200": tinycolor__default.default(bg).lighten(5).toHexString(),
    // Slightly lighter
    "base-300": tinycolor__default.default(bg).lighten(12).toHexString(),
    // More lighter
    "base-content": fg,
    // Main text color
    neutral: tinycolor__default.default(bg).lighten(15).desaturate(20).toHexString(),
    // Muted neutral
    "neutral-content": tinycolor__default.default(fg).darken(20).desaturate(20).toHexString(),
    // Darkened text
    primary,
    secondary: tinycolor__default.default(primary).darken(20).toHexString(),
    // Darker secondary
    accent: tinycolor__default.default(primary).triad()[1].toHexString()
    // Triad accent
  };
};
var themes = [
...
var useThemeStore = zustand.create((set, get) => {
  ...
  return {
    ...
    updateAppTheme: (color) => {
      ...
      if (isWebAppPlatform()) {
        ...
        (_a = document.querySelector('meta[name="theme-color"]')) == null ? void 0 : _a.setAttribute("content", palette[color]);
      }
    },
...
var useSettingsStore = zustand.create((set) => ({
  settings: {},
...
var useTheme = () => {
  ...
  React42.useEffect(() => {
    var _a;
    const customThemes = ((_a = settings.globalReadSettings) == null ? void 0 : _a.customThemes) ?? [];
...
var eventDispatcher = typeof window === "undefined" ? null : new EventEmitter();
...
var useBookDataStore = zustand.create((set, get) => ({
  ...
  getConfig: (key) => {
    ...
    return ((_a = get().booksData[id]) == null ? void 0 : _a.config) || null;
  },
  setConfig: (key, partialConfig) => {
    set((state) => {
      var _a;
      const id = key.split("-")[0];
      const config2 = ((_a = state.booksData[id]) == null ? void 0 : _a.config) || {};
      Object.assign(config2, partialConfig);
      return {
        booksData: {
          ...state.booksData,
          [id]: {
  ...
  saveConfig: async (envConfig, bookKey, config2, settings) => {
    console.log(`\u{1F4BE} Saving book config for ${bookKey}`, config2);
    const appService = await envConfig.getAppService();
    ...
    book.progress = config2.progress;
    book.updatedAt = Date.now();
    config2.updatedAt = Date.now();
    try {
      await appService.saveBookConfig(book, config2, settings);
...
var findTocItemBS = (toc, cfi) => {
  ...
  while (left <= right) {
    ...
    const currentCfi = ((_a = toc[mid]) == null ? void 0 : _a.cfi) || "";
...
init_document();
var useReaderStore = zustand.create((set, get) => ({
  viewStates: {},
  bookKeys: [],
  hoveredBookKey: null,
  ...
  getView: (key) => {
    var _a;
    return key && ((_a = get().viewStates[key]) == null ? void 0 : _a.view) || null;
  },
  ...
  initViewState: async (envConfig, id, key, isPrimary = true) => {
    ...
    try {
      if (!bookData) {
        ...
        const { file, config: config3 } = content;
        console.log("Loading book", key);
        const { book: loadedBookDoc } = await new DocumentLoader(file).open();
        const bookDoc = loadedBookDoc;
        if (((_a = bookDoc.toc) == null ? void 0 : _a.length) && ((_b = bookDoc.sections) == null ? void 0 : _b.length)) {
        ...
        useBookDataStore.setState((state) => ({
          booksData: {
            ...state.booksData,
            [id]: { id, book, file, config: config3, bookDoc }
          }
        }));
      }
      const booksData2 = useBookDataStore.getState().booksData;
      const config2 = (_c = booksData2[id]) == null ? void 0 : _c.config;
      const configViewSettings = config2.viewSettings;
      set((state) => ({
        viewStates: {
          ...state.viewStates,
          [key]: {
  ...
  getViewSettings: (key) => {
    var _a;
    return ((_a = get().viewStates[key]) == null ? void 0 : _a.viewSettings) || null;
  },
  ...
  getProgress: (key) => {
    var _a;
    return ((_a = get().viewStates[key]) == null ? void 0 : _a.progress) || null;
  },
  setProgress: (key, location, tocItem, section, pageinfo, range) => set((state) => {
    ...
    const progress = [((pageinfo == null ? void 0 : pageinfo.next) ?? (pageinfo == null ? void 0 : pageinfo.current) ?? 0) + 1, (pageinfo == null ? void 0 : pageinfo.total) ?? 1];
    const book = bookData.book;
    if (book) {
    ...
    return {
      viewStates: {
        ...
        [key]: {
          ...
          progress: {
            location,
            sectionId: (tocItem == null ? void 0 : tocItem.id) ?? "",
            sectionHref: (tocItem == null ? void 0 : tocItem.href) ?? "",
            sectionLabel: (tocItem == null ? void 0 : tocItem.label) ?? "",
            section,
            pageinfo,
            range
          }
...
var useSidebarStore = zustand.create((set) => ({
  sideBarBookKey: null,
  sideBarWidth: "",
...
// src/utils/webWindow.js
var handleOnCloseWindow = (handler) => {
...
// src/app/reader/components/ReaderContent.jsx
init_misc();
...
var useNotebookStore = zustand.create((set, get) => ({
  notebookWidth: "",
...
var useShortcuts = (actions, dependencies = []) => {
  ...
  const processKeyEvent = (key, ctrlKey, altKey, metaKey, shiftKey) => {
    ...
    for (const [actionName, actionHandler] of Object.entries(actions)) {
      ...
      if (handler && (shortcutList == null ? void 0 : shortcutList.some(
...
// src/app/reader/hooks/useBookShortcuts.js
init_constants();
var useBookShortcuts = ({ sideBarBookKey, bookKeys }) => {
  ...
  const fontSize = (viewSettings == null ? void 0 : viewSettings.defaultFontSize) ?? 16;
  const lineHeight = (viewSettings == null ? void 0 : viewSettings.lineHeight) ?? 1.6;
  ...
  const toggleScrollMode = () => {
    ...
    if (viewSettings2 && sideBarBookKey) {
      ...
      (_a = getView(sideBarBookKey)) == null ? void 0 : _a.renderer.setAttribute("flow", flowMode);
    }
  };
  const switchSideBar = () => {
  ...
  const goLeft = () => {
    var _a;
    (_a = getView(sideBarBookKey)) == null ? void 0 : _a.goLeft();
  };
  const goRight = () => {
    var _a;
    (_a = getView(sideBarBookKey)) == null ? void 0 : _a.goRight();
  };
  const goPrev = () => {
    var _a;
    (_a = getView(sideBarBookKey)) == null ? void 0 : _a.prev(distance);
  };
  const goNext = () => {
    var _a;
    (_a = getView(sideBarBookKey)) == null ? void 0 : _a.next(distance);
  };
  const goBack = () => {
    var _a;
    (_a = getView(sideBarBookKey)) == null ? void 0 : _a.history.back();
  };
  ...
  const goForward = () => {
    var _a;
    (_a = getView(sideBarBookKey)) == null ? void 0 : _a.history.forward();
  };
  const reloadPage = () => {
    window.location.reload();
  };
  ...
  const zoomIn = () => {
    ...
    const view = getView(sideBarBookKey);
    if (!((_a = view == null ? void 0 : view.renderer) == null ? void 0 : _a.setStyles)) return;
    const viewSettings2 = getViewSettings(sideBarBookKey);
    const zoomLevel = viewSettings2.zoomLevel + ZOOM_STEP;
    viewSettings2.zoomLevel = Math.min(zoomLevel, MAX_ZOOM_LEVEL);
    setViewSettings(sideBarBookKey, viewSettings2);
    (_c = view == null ? void 0 : (_b = view.renderer).setStyles) == null ? void 0 : _c.call(_b, getStyles(viewSettings2));
  };
  const zoomOut = () => {
    var _a, _b, _c;
    if (!sideBarBookKey) return;
...
// src/i18n/i18n.js
var import_i18next_scanner_config = __toESM(require_i18next_scanner_config());
var { options } = import_i18next_scanner_config.default;
i18n__default.default.use(HttpApi__default.default).use(LanguageDetector__default.default).use(reactI18next.initReactI18next).init({
...
i18n__default.default.on("languageChanged", (lng) => {
...
Alert.propTypes = {
  title: PropTypes13__default.default.string.isRequired,
  message: PropTypes13__default.default.string.isRequired,
  onCancel: PropTypes13__default.default.func.isRequired,
  onConfirm: PropTypes13__default.default.func.isRequired
};
...
Spinner.propTypes = {
  loading: PropTypes13__default.default.bool.isRequired
};
...
var useResponsiveSize = (baseSize) => {
  const isPhone = reactResponsive.useMediaQuery({ maxWidth: 480 });
  const isTablet = reactResponsive.useMediaQuery({ minWidth: 481, maxWidth: 1024 });
...
var Dialog = ({
  ...
  const handleDragEnd = (data) => {
    ...
    if (data.velocity > VELOCITY_THRESHOLD || data.velocity >= 0 && data.clientY >= window.innerHeight * snapLower) {
      ...
      if (appService == null ? void 0 : appService.hasHaptics) {
    ...
    /* @__PURE__ */ React42__namespace.default.createElement(
      "div",
      {
        className: clsx8__default.default(
          ...
          (appService == null ? void 0 : appService.hasSafeAreaInset) && isFullHeightInMobile && "pt-[env(safe-area-inset-top)] sm:pt-0",
          boxClassName
        ),
        style: snapHeight ? {
...
Dialog.propTypes = {
  id: PropTypes13__default.default.string,
  isOpen: PropTypes13__default.default.bool.isRequired,
  children: PropTypes13__default.default.node.isRequired,
  snapHeight: PropTypes13__default.default.number,
  header: PropTypes13__default.default.node,
  title: PropTypes13__default.default.string,
  className: PropTypes13__default.default.string,
  bgClassName: PropTypes13__default.default.string,
  boxClassName: PropTypes13__default.default.string,
  contentClassName: PropTypes13__default.default.string,
  onClose: PropTypes13__default.default.func.isRequired
};
...
var BookDetailModal = ({ book, isOpen, onClose }) => {
  ...
  const confirmDelete = async () => {
    await (appService == null ? void 0 : appService.deleteBook(book, !!book.uploadedAt));
    const books = await (appService == null ? void 0 : appService.loadLibraryBooks()) || [];
    const bookIndex = books.findIndex((b) => b.hash === book.hash);
    if (bookIndex >= 0) {
      books[bookIndex] = { ...book, deletedAt: Date.now() };
      await (appService == null ? void 0 : appService.saveLibraryBooks(books));
    }
    handleClose();
    ...
    /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "flex w-full select-text items-center justify-center" }, /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "relative w-full rounded-lg" }, /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "mb-10 flex h-40 items-start" }, /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "book-cover relative mr-10 aspect-[28/41] h-40 items-end shadow-lg" }, /* @__PURE__ */ React42__namespace.default.createElement(
      "img",
      {
        ...
        onError: (e) => {
          ...
          (_a = e.target.nextElementSibling) == null ? void 0 : _a.classList.remove("invisible");
        }
      }
    ), /* @__PURE__ */ React42__namespace.default.createElement(
      "div",
...
BookDetailModal.propTypes = {
  book: PropTypes13__default.default.shape({
    title: PropTypes13__default.default.string.isRequired,
    author: PropTypes13__default.default.string,
    coverImageUrl: PropTypes13__default.default.string,
    hash: PropTypes13__default.default.string.isRequired,
    lastUpdated: PropTypes13__default.default.number,
    uploadedAt: PropTypes13__default.default.number
  }).isRequired,
  isOpen: PropTypes13__default.default.bool.isRequired,
  onClose: PropTypes13__default.default.func.isRequired
};
...
init_book();
var useTrafficLightStore = zustand.create((set, get) => {
  return {
    appService: void 0,
...
// src/app/reader/components/sidebar/Header.jsx
...
var TOCItemView = ({ bookKey, item, depth, expandedItems }) => {
  ...
  const handleClickItem = (event) => {
    ...
    if (item.href) {
      (_a = getView(bookKey)) == null ? void 0 : _a.goTo(item.href);
    }
  };
...
var TOCView = ({ bookKey, toc }) => {
  ...
  const scrollToProgress = (progress2) => {
    ...
    const currentItem = (_a = viewRef.current) == null ? void 0 : _a.querySelector(`[data-href="${hrefMd5}"]`);
    if (currentItem) {
...
var useScrollToItem = (cfi, progress) => {
  ...
  React42.useEffect(() => {
    ...
    const end = CFI4__namespace.collapse(location, true);
    const isCurrent2 = CFI4__namespace.compare(cfi, start) >= 0 && CFI4__namespace.compare(cfi, end) <= 0;
...
// src/app/reader/components/sidebar/BooknoteItem.jsx
var BooknoteItem = ({ bookKey, item }) => {
  ...
  const handleClickItem = (event) => {
    ...
    (_a = getView(bookKey)) == null ? void 0 : _a.goTo(cfi);
    if (note) {
  ...
  const deleteNote = (note2) => {
    ...
    if (!config2) return;
    const { booknotes = [] } = config2;
    booknotes.forEach((item2) => {
      if (item2.id === note2.id) {
        item2.deletedAt = Date.now();
        const views = getViewsById(bookKey.split("-")[0]);
        views.forEach((view) => view == null ? void 0 : view.addAnnotation(item2, true));
      }
    });
...
// src/app/reader/components/sidebar/BooknoteView.jsx
var BooknoteView = ({ type, bookKey, toc }) => {
  ...
  for (const booknote of booknotes) {
    ...
    const href = (tocItem == null ? void 0 : tocItem.href) || "";
    const label = (tocItem == null ? void 0 : tocItem.label) || "";
    const id = (tocItem == null ? void 0 : tocItem.id) || 0;
  ...
  Object.values(booknoteGroups).forEach((group) => {
    group.booknotes.sort((a, b) => {
      return CFI4__namespace.compare(a.cfi, b.cfi);
    });
  });
...
// src/app/reader/components/sidebar/Content.jsx
var SidebarContent = ({ bookDoc, sideBarBookKey }) => {
  ...
  const [activeTab, setActiveTab] = React42.useState(((_a = config2 == null ? void 0 : config2.viewSettings) == null ? void 0 : _a.sideBarTab) || "toc");
  ...
  React42.useEffect(() => {
    if (!sideBarBookKey) return;
    const config3 = getConfig(sideBarBookKey);
    setActiveTab(config3.viewSettings.sideBarTab);
  }, [sideBarBookKey]);
  const handleTabChange = (tab) => {
    ...
    const timeout = setTimeout(() => {
      setFade(false);
      setTargetTab(tab);
      setConfig(sideBarBookKey, config3);
      clearTimeout(timeout);
    }, 300);
    setActiveTab(tab);
    const config3 = getConfig(sideBarBookKey);
    config3.viewSettings.sideBarTab = tab;
  };
  return /* @__PURE__ */ React42__namespace.default.createElement(React42__namespace.default.Fragment, null, /* @__PURE__ */ React42__namespace.default.createElement(
    ...
    "div",
    {
      className: clsx8__default.default(
        "flex-shrink-0",
        (appService == null ? void 0 : appService.hasSafeAreaInset) && "pb-[calc(env(safe-area-inset-bottom)/2)]"
      )
    },
...
var Dropdown = ({
  ...
  const toggleDropdown = () => {
    ...
    setIsOpen(newIsOpen);
    onToggle == null ? void 0 : onToggle(newIsOpen);
  };
  const setIsDropdownOpen = (isOpen2) => {
    setIsOpen(isOpen2);
    onToggle == null ? void 0 : onToggle(isOpen2);
  };
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "dropdown-container" }, isOpen && /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "fixed inset-0 bg-transparent", onClick: () => setIsDropdownOpen(false) }), /* @__PURE__ */ React42__namespace.default.createElement("div", { className: clsx8__default.default("dropdown", className) }, /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      ...
      className: clsx8__default.default("dropdown-toggle", buttonClassName, isOpen && "bg-base-300/50")
    },
    toggleButton
...
Dropdown.propTypes = {
  className: PropTypes13__default.default.string,
  menuClassName: PropTypes13__default.default.string,
  buttonClassName: PropTypes13__default.default.string,
  toggleButton: PropTypes13__default.default.node.isRequired,
  children: PropTypes13__default.default.element.isRequired,
  onToggle: PropTypes13__default.default.func
};
...
var SearchOptions = ({
  ...
  const updateConfig = (key, value) => {
    ...
    setIsDropdownOpen == null ? void 0 : setIsDropdownOpen(false);
  };
  return /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
...
// src/app/reader/components/sidebar/SearchBar.jsx
var MINIMUM_SEARCH_TERM_LENGTH_DEFAULT = 2;
var MINIMUM_SEARCH_TERM_LENGTH_CJK = 1;
var SearchBar = ({ isVisible, bookKey, searchTerm: term, onSearchResultChange }) => {
  const _ = useTranslation();
  const { envConfig } = useEnv();
  const { settings } = useSettingsStore();
  ...
  const handleSearchConfigChange = (searchConfig2) => {
    config2.searchConfig = searchConfig2;
    saveConfig(envConfig, bookKey, config2, settings);
    handleSearchTermChange(searchTerm);
  };
  ...
  const resetSearch = () => {
    ...
    view == null ? void 0 : view.clearSearch();
  };
...
var SearchResultItem = ({ bookKey, cfi, excerpt, onSelectResult }) => {
  const { getProgress } = useReaderStore();
  const progress = getProgress(bookKey);
  const { isCurrent, viewRef } = useScrollToItem_default(cfi, progress);
  return /* @__PURE__ */ React42__namespace.default.createElement(
...
// src/app/reader/components/sidebar/SideBar.jsx
...
var SideBar = ({ onGoToLibrary }) => {
  ...
  const handleToggleSearchBar = () => {
    ...
    if (isSearchBarVisible) {
      ...
      setSearchTerm("");
      (_a = getView(sideBarBookKey)) == null ? void 0 : _a.clearSearch();
    }
  };
  ...
  const handleSearchResultClick = (cfi) => {
    var _a;
    onNavigateEvent();
    (_a = getView(sideBarBookKey)) == null ? void 0 : _a.goTo(cfi);
  };
  ...
  return isSideBarVisible ? /* @__PURE__ */ React42__namespace.default.createElement(React42__namespace.default.Fragment, null, /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      className: clsx8__default.default(
        ...
        (appService == null ? void 0 : appService.isIOSApp) ? "h-[100vh]" : "h-full",
        (appService == null ? void 0 : appService.hasSafeAreaInset) && "pt-[env(safe-area-inset-top)]",
        (appService == null ? void 0 : appService.hasRoundedWindow) && "rounded-window-top-left rounded-window-bottom-left",
        !isSideBarPinned && "shadow-2xl"
      ),
      dir: (viewSettings == null ? void 0 : viewSettings.rtl) && languageDir === "rtl" ? "rtl" : "ltr",
      style: {
        width: `${sideBarWidth}`,
    ...
    /* @__PURE__ */ React42__namespace.default.createElement(
      "div",
      {
        className: clsx8__default.default("drag-handle absolute right-0 top-0 h-full w-1 cursor-col-resize", {
          "hidden": isMobile
        }),
    ...
    "div",
    {
      className: "overlay fixed inset-0 z-10 bg-black/80",
...
init_misc();
init_book();
var NotebookHeader = ({ isPinned, handleClose, handleTogglePin }) => {
  const _ = useTranslation();
  return /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "flex h-11 items-center justify-between border-b px-3" }, /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "flex items-center gap-2" }, /* @__PURE__ */ React42__namespace.default.createElement(
    "button",
    {
    ...
    /* @__PURE__ */ React42__namespace.default.createElement(
      md.MdPushPin,
      {
        className: clsx8__default.default("h-5 w-5", isPinned && "rotate-45")
      }
    )
  ), /* @__PURE__ */ React42__namespace.default.createElement("span", { className: "font-size-sm font-medium" }, _("Notebook"))), /* @__PURE__ */ React42__namespace.default.createElement(
    "button",
    {
      className: "btn btn-ghost btn-sm h-8 min-h-8 w-8 p-0",
      onClick: handleClose,
    ...
    /* @__PURE__ */ React42__namespace.default.createElement(md.MdClose, { className: "h-5 w-5" })
  ));
};
...
var NoteEditor = ({ onSave, onEdit }) => {
  ...
  const handleSave = () => {
    var _a;
    const text = (_a = textareaRef.current) == null ? void 0 : _a.value;
    if (!text) return;
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "my-2" }, /* @__PURE__ */ React42__namespace.default.createElement(
    "textarea",
    {
      ref: textareaRef,
      className: "textarea textarea-bordered w-full resize-none",
      placeholder: _("Type your note here..."),
      defaultValue: (notebookEditAnnotation == null ? void 0 : notebookEditAnnotation.note) || "",
      onKeyDown: handleKeyDown
    }
  ), /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "flex justify-end", dir: "ltr" }, /* @__PURE__ */ React42__namespace.default.createElement(
    "button",
    {
...
// src/app/reader/components/notebook/Notebook.jsx
...
var Notebook = () => {
  ...
  const handleSaveNote = (selection, note) => {
    ...
    const view = getView(sideBarBookKey);
    const config3 = getConfig(sideBarBookKey);
    const cfi = view == null ? void 0 : view.getCFI(selection.index, selection.range);
    if (!cfi) return;
    const { booknotes: annotations = [] } = config3;
    const annotation = {
      id: uniqueId(),
      type: "annotation",
      cfi,
  ...
  return isNotebookVisible ? /* @__PURE__ */ React42__namespace.default.createElement(React42__namespace.default.Fragment, null, !isNotebookPinned && /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "overlay fixed inset-0 z-10 bg-black/20", onClick: handleClickOverlay }), /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      className: clsx8__default.default(
        ...
        (appService == null ? void 0 : appService.isIOSApp) ? "h-[100vh]" : "h-full",
        (appService == null ? void 0 : appService.hasSafeAreaInset) && "pt-[env(safe-area-inset-top)]",
        (appService == null ? void 0 : appService.hasRoundedWindow) && "rounded-window-top-right rounded-window-bottom-right",
...
// src/app/reader/components/FoliateViewer.jsx
init_document();
    ...
    if (msg instanceof MessageEvent) {
      if (msg.data && msg.data.bookKey === bookKey) {
        ...
        if (msg.data.type === "iframe-single-click") {
              ...
              if (viewSettings.disableClick || screenX >= centerStartX && screenX <= centerEndX) {
                ...
                if (!viewSettings.disableClick && screenX >= viewCenterX) {
                  if (viewSettings.swapClickArea) {
                    (_a = viewRef.current) == null ? void 0 : _a.goLeft();
                  } else {
                    (_b = viewRef.current) == null ? void 0 : _b.goRight();
                  }
                } else if (!viewSettings.disableClick && screenX < viewCenterX) {
                  if (viewSettings.swapClickArea) {
                    (_c = viewRef.current) == null ? void 0 : _c.goRight();
                  } else {
                    (_d = viewRef.current) == null ? void 0 : _d.goLeft();
                  }
                }
              }
            }
          ...
          if (deltaY > 0) {
            (_e = viewRef.current) == null ? void 0 : _e.next(1);
          } else if (deltaY < 0) {
            (_f = viewRef.current) == null ? void 0 : _f.prev(1);
          }
        } else if (msg.data.type === "iframe-mouseup") {
          if (msg.data.button === 3) {
            (_g = viewRef.current) == null ? void 0 : _g.history.back();
          } else if (msg.data.button === 4) {
            (_h = viewRef.current) == null ? void 0 : _h.history.forward();
          }
        }
      }
    } else {
      ...
      if (clientX < leftThreshold) {
        (_i = viewRef.current) == null ? void 0 : _i.goLeft();
      } else if (clientX > rightThreshold) {
        (_j = viewRef.current) == null ? void 0 : _j.goRight();
      }
    }
  };
  React42.useEffect(() => {
...
var useFoliateEvents = (view, handlers) => {
  const onLoad = handlers == null ? void 0 : handlers.onLoad;
  const onRelocate = handlers == null ? void 0 : handlers.onRelocate;
  const onLinkClick = handlers == null ? void 0 : handlers.onLinkClick;
  const onRendererRelocate = handlers == null ? void 0 : handlers.onRendererRelocate;
  const onDrawAnnotation = handlers == null ? void 0 : handlers.onDrawAnnotation;
  const onShowAnnotation = handlers == null ? void 0 : handlers.onShowAnnotation;
  React42.useEffect(() => {
    if (!view) return;
...
function useSync(bookKey) {
  ...
  const { getConfig, setConfig } = useBookDataStore();
  const config2 = bookKey ? getConfig(bookKey) : null;
  ...
  React42.useEffect(() => {
    if (!settings || !config2) return;
    if (lastSyncedAtInited.current) return;
    lastSyncedAtInited.current = true;
    const lastSyncedBooksAt = settings.lastSyncedAtBooks ?? 0;
    const lastSyncedConfigsAt = (config2 == null ? void 0 : config2.lastSyncedAtConfig) ?? settings.lastSyncedAtConfigs ?? 0;
    const lastSyncedNotesAt = (config2 == null ? void 0 : config2.lastSyncedAtNotes) ?? settings.lastSyncedAtNotes ?? 0;
      ...
      switch (type) {
        ...
        case "configs":
          if (!bookId) {
            ...
            setSettings(settings2);
          } else if (bookKey && config2) {
            config2.lastSyncedAtConfig = maxTime;
            setConfig(bookKey, config2);
          }
          break;
        case "notes":
          if (!bookId) {
            ...
            config2.lastSyncedAtNotes = maxTime;
            setConfig(bookKey, config2);
          }
          break;
      }
  ...
  const syncBooks = async (books, op = "both") => {
    if ((op === "push" || op === "both") && (books == null ? void 0 : books.length)) {
  ...
  const syncConfigs = async (bookConfigs, bookId, op = "both") => {
    if ((op === "push" || op === "both") && (bookConfigs == null ? void 0 : bookConfigs.length)) {
  ...
  const syncNotes = async (bookNotes, bookId, op = "both") => {
    if ((op === "push" || op === "both") && (bookNotes == null ? void 0 : bookNotes.length)) {
  ...
  React42.useEffect(() => {
    if (!syncing && syncResult) {
      ...
      const books = dbBooks == null ? void 0 : dbBooks.map(
        (dbBook) => transformsFromDB["books"](dbBook)
      );
      const configs = dbBookConfigs == null ? void 0 : dbBookConfigs.map(
        (dbBookConfig) => transformsFromDB["configs"](dbBookConfig)
      );
      const notes = dbBookNotes == null ? void 0 : dbBookNotes.map(
...
// src/app/reader/hooks/useProgressSync.js
init_serializer();
init_document();
init_constants();
var useProgressSync = (bookKey) => {
  ...
  const pushConfig = (bookKey2, config3) => {
    if (!config3) return;
    const bookHash = bookKey2.split("-")[0];
    const newConfig = { bookHash, ...config3 };
  ...
  const syncConfig = () => {
    if (!configSynced.current) {
      ...
      if (config2 && config2.progress && config2.progress[0] > 0) {
        pushConfig(bookKey, config2);
      }
    }
  };
  ...
  React42.useEffect(() => {
    if (!(config2 == null ? void 0 : config2.location)) return;
    const now = Date.now();
  ...
  React42.useEffect(() => {
    if (!configSynced.current && syncedConfigs) {
      ...
      if (syncedConfig) {
        const configCFI = config2 == null ? void 0 : config2.location;
...
// src/app/reader/hooks/useProgressAutoSave.js
var useProgressAutoSave = (bookKey) => {
  ...
  const saveBookConfig = React42.useCallback(
    throttle(async () => {
      ...
      await saveConfig(envConfig, bookKey, config2, settings);
    }, 1e4),
    []
  );
  React42.useEffect(() => {
...
// src/app/reader/utils/iframeEventHandlers.js
init_constants();
init_misc();
var doubleClickEnabled = typeof window === "undefined" ? true : (
...
// src/services/transformers/translate.js
...
// src/services/transformers/punctuation.js
var punctuationMap = {
  '"': "\uFE43",
  '"': "\uFE44",
  "'": "\uFE41",
...
// src/services/transformers/index.js
...
// src/app/reader/components/FoliateViewer.jsx
var FoliateViewer = ({ bookKey, bookDoc, config: config2 }) => {
  ...
  const docLoadHandler = (event) => {
    ...
    if (detail.doc) {
      const writingDir = ((_a = viewRef.current) == null ? void 0 : _a.renderer.setStyles) && getDirection(detail.doc);
      const viewSettings2 = getViewSettings(bookKey);
      viewSettings2.vertical = (writingDir == null ? void 0 : writingDir.vertical) || viewSettings2.writingMode.includes("vertical") || false;
      viewSettings2.rtl = (writingDir == null ? void 0 : writingDir.rtl) || getDirFromUILanguage() === "rtl" || viewSettings2.writingMode.includes("rl") || false;
  ...
  const docRelocateHandler = (event) => {
    ...
    if (detail.reason === "scroll") {
      const renderer = (_a = viewRef.current) == null ? void 0 : _a.renderer;
      const viewSettings2 = getViewSettings(bookKey);
      if (renderer && viewSettings2.continuousScroll) {
        if (renderer.start <= 0) {
          (_b = viewRef.current) == null ? void 0 : _b.prev(1);
        } else if (renderer.end + 1 >= renderer.viewSize) {
          (_c = viewRef.current) == null ? void 0 : _c.next(1);
        }
      }
    }
  };
  ...
  React42.useEffect(() => {
    ...
    if (viewRef.current && viewRef.current.renderer) {
      const viewSettings2 = getViewSettings(bookKey);
      (_b = (_a = viewRef.current.renderer).setStyles) == null ? void 0 : _b.call(_a, getStyles(viewSettings2));
    }
  ...
  React42.useEffect(() => {
    ...
    const openBook = async () => {
      ...
      (_a = containerRef.current) == null ? void 0 : _a.appendChild(view);
      ...
      const { book } = view;
      (_b = book.transformTarget) == null ? void 0 : _b.addEventListener("data", docTransformHandler);
      (_d = (_c = view.renderer).setStyles) == null ? void 0 : _d.call(_c, getStyles(viewSettings));
      const isScrolled = viewSettings.scrolled;
      ...
      const lastLocation = config2.location;
      if (lastLocation) {
...
WindowButton.propTypes = {
  id: PropTypes13__default.default.string.isRequired,
  onClick: PropTypes13__default.default.func.isRequired,
  ariaLabel: PropTypes13__default.default.string.isRequired,
  children: PropTypes13__default.default.node.isRequired
};
var WindowButtons = ({
  className,
...
Button.propTypes = {
  icon: PropTypes13__default.default.node.isRequired,
  onClick: PropTypes13__default.default.func.isRequired,
  disabled: PropTypes13__default.default.bool,
  tooltip: PropTypes13__default.default.string,
  tooltipDirection: PropTypes13__default.default.oneOf(["top", "bottom", "left", "right"]),
  className: PropTypes13__default.default.string
};
var Button_default = Button;

// src/app/reader/components/SidebarToggler.jsx
...
var BookmarkToggler = ({ bookKey }) => {
  ...
  const toggleBookmark = () => {
    var _a;
    const { booknotes: bookmarks = [] } = config2;
    const { location: cfi, range } = progress;
    if (!cfi) return;
    if (!isBookmarked) {
      setIsBookmarked(true);
      const text = ((_a = range == null ? void 0 : range.startContainer.textContent) == null ? void 0 : _a.slice(0, 128)) || "";
      ...
      const start = CFI4__namespace.collapse(cfi);
      const end = CFI4__namespace.collapse(cfi, true);
      bookmarks.forEach((item) => {
        if (item.type === "bookmark" && CFI4__namespace.compare(start, item.cfi) * CFI4__namespace.compare(end, item.cfi) <= 0) {
  ...
  React42.useEffect(() => {
    ...
    const start = CFI4__namespace.collapse(cfi);
    const end = CFI4__namespace.collapse(cfi, true);
    const locationBookmarked = booknotes.filter((booknote) => booknote.type === "bookmark" && !booknote.deletedAt).some((item) => CFI4__namespace.compare(start, item.cfi) * CFI4__namespace.compare(end, item.cfi) <= 0);
...
// src/app/reader/components/ViewMenu.jsx
init_constants();
init_config();

// src/app/reader/utils/viewSettingsHelper.js
var saveViewSettings = async (envConfig, bookKey, key, value, skipGlobal = false, applyStyles = true) => {
  ...
  if (viewSettings[key] !== value) {
    ...
    if (applyStyles) {
      const view = getView(bookKey);
      (_b = view == null ? void 0 : (_a = view.renderer).setStyles) == null ? void 0 : _b.call(_a, getStyles(viewSettings));
    }
  }
...
MenuItem.propTypes = {
  label: PropTypes13__default.default.string.isRequired,
  labelClass: PropTypes13__default.default.string,
  shortcut: PropTypes13__default.default.string,
  disabled: PropTypes13__default.default.bool,
  noIcon: PropTypes13__default.default.bool,
  icon: PropTypes13__default.default.node,
  children: PropTypes13__default.default.node,
  onClick: PropTypes13__default.default.func
};
...
var ViewMenu = ({
  ...
  React42.useEffect(() => {
    ...
    (_a = getView(bookKey)) == null ? void 0 : _a.renderer.setAttribute("flow", isScrolledMode ? "scrolled" : "paginated");
    (_b = getView(bookKey)) == null ? void 0 : _b.renderer.setAttribute(
      "max-inline-size",
      `${getMaxInlineSize(viewSettings)}px`
    );
    (_e = (_c = getView(bookKey)) == null ? void 0 : (_d = _c.renderer).setStyles) == null ? void 0 : _e.call(_d, getStyles(viewSettings));
    setViewSettings(bookKey, viewSettings);
  }, [isScrolledMode]);
  React42.useEffect(() => {
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement(
    ...
    /* @__PURE__ */ React42__namespace.default.createElement("hr", { className: "border-base-300 my-1" }),
    (appService == null ? void 0 : appService.hasWindow) && /* @__PURE__ */ React42__namespace.default.createElement(MenuItem_default, { label: _("Fullscreen"), onClick: handleFullScreen }),
    /* @__PURE__ */ React42__namespace.default.createElement(
      MenuItem_default,
      {
...
// src/app/reader/components/HeaderBar.jsx
var HeaderBar = ({
  bookKey,
  bookTitle,
  isTopLeft,
  ...
  React42.useEffect(() => {
    if (!(appService == null ? void 0 : appService.hasTrafficLight)) return;
    initializeTrafficLightStore(appService);
    initializeTrafficLightListeners();
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      ...
      className: clsx8__default.default(
        ...
        (appService == null ? void 0 : appService.hasRoundedWindow) && "rounded-window-top-right",
        !isSideBarVisible && (appService == null ? void 0 : appService.hasRoundedWindow) && "rounded-window-top-left",
    ...
    /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "bg-base-100 z-20 ml-auto flex h-full items-center space-x-4" }, /* @__PURE__ */ React42__namespace.default.createElement(SettingsToggler_default, null), /* @__PURE__ */ React42__namespace.default.createElement(NotebookToggler_default, { bookKey }), /* @__PURE__ */ React42__namespace.default.createElement(
      ...
      WindowButtons_default,
      {
        ...
        headerRef,
        showMinimize: bookKeys.length == 1 && !isTrafficLightVisible && (appService == null ? void 0 : appService.appPlatform) !== "web",
        showMaximize: bookKeys.length == 1 && !isTrafficLightVisible && (appService == null ? void 0 : appService.appPlatform) !== "web",
...
Slider.propTypes = {
  min: PropTypes13__default.default.number,
  max: PropTypes13__default.default.number,
  step: PropTypes13__default.default.number,
  initialValue: PropTypes13__default.default.number,
  heightPx: PropTypes13__default.default.number,
  minLabel: PropTypes13__default.default.string,
  maxLabel: PropTypes13__default.default.string,
  bubbleElement: PropTypes13__default.default.node,
  bubbleLabel: PropTypes13__default.default.string,
  className: PropTypes13__default.default.string,
  minClassName: PropTypes13__default.default.string,
  maxClassName: PropTypes13__default.default.string,
  bubbleClassName: PropTypes13__default.default.string,
  onChange: PropTypes13__default.default.func
};
var Slider_default = Slider;

// src/app/reader/components/FooterBar.jsx
var FooterBar = ({
  bookKey,
  bookFormat,
  section,
  ...
  const handleProgressChange = (value) => {
    view == null ? void 0 : view.goToFraction(value / 100);
  };
  ...
  const handleMarginChange = (value) => {
    ...
    view == null ? void 0 : view.renderer.setAttribute("margin", `${marginPx}px`);
    view == null ? void 0 : view.renderer.setAttribute("gap", `${gapPercent}%`);
    if (viewSettings == null ? void 0 : viewSettings.scrolled) {
      view == null ? void 0 : view.renderer.setAttribute("flow", "scrolled");
    }
  };
  ...
  const handleGoPrev = () => {
    view == null ? void 0 : view.goLeft();
  };
  const handleGoNext = () => {
    view == null ? void 0 : view.goRight();
  };
  const handleGoBack = () => {
    view == null ? void 0 : view.history.back();
  };
  const handleGoForward = () => {
    view == null ? void 0 : view.history.forward();
  };
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement(React42__namespace.default.Fragment, null, /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      className: clsx8__default.default(
        ...
        (viewSettings == null ? void 0 : viewSettings.vertical) && (viewSettings == null ? void 0 : viewSettings.scrolled) && "sm:!bottom-3 sm:!h-7"
      ),
      onMouseEnter: () => !(appService == null ? void 0 : appService.isMobile) && setHoveredBookKey(bookKey),
      onTouchStart: () => !(appService == null ? void 0 : appService.isMobile) && setHoveredBookKey(bookKey)
    }
  ), /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      className: clsx8__default.default(
        ...
        (appService == null ? void 0 : appService.hasRoundedWindow) && "rounded-window-bottom-right",
        !isSideBarVisible && (appService == null ? void 0 : appService.hasRoundedWindow) && "rounded-window-bottom-left",
        isHoveredAnim && "hover-bar-anim",
        // show scroll bar when vertical and scrolled in desktop
        (viewSettings == null ? void 0 : viewSettings.vertical) && (viewSettings == null ? void 0 : viewSettings.scrolled) && "sm:!bottom-3 sm:!h-7",
        isVisible ? `pointer-events-auto translate-y-0 opacity-100` : `pointer-events-none translate-y-full opacity-0 sm:translate-y-0`
      ),
      dir: (viewSettings == null ? void 0 : viewSettings.rtl) ? "rtl" : "ltr",
    ...
    /* @__PURE__ */ React42__namespace.default.createElement(
      "div",
      {
        ...
        style: {
          bottom: (appService == null ? void 0 : appService.hasSafeAreaInset) ? "calc(env(safe-area-inset-bottom) + 64px)" : "64px"
        }
      },
      /* @__PURE__ */ React42__namespace.default.createElement(
        Button_default,
        {
          icon: (viewSettings == null ? void 0 : viewSettings.rtl) ? /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowRightWideLine, null) : /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowLeftWideLine, null),
          onClick: (viewSettings == null ? void 0 : viewSettings.rtl) ? handleGoNext : handleGoPrev,
          tooltip: (viewSettings == null ? void 0 : viewSettings.rtl) ? _("Go Right") : _("Go Left")
        }
      ),
      /* @__PURE__ */ React42__namespace.default.createElement(
        Button_default,
        {
          icon: (viewSettings == null ? void 0 : viewSettings.rtl) ? /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowGoForwardLine, null) : /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowGoBackLine, null),
          onClick: handleGoBack,
          tooltip: _("Go Back"),
          disabled: !(view == null ? void 0 : view.history.canGoBack)
        }
      ),
      /* @__PURE__ */ React42__namespace.default.createElement(
        Button_default,
        {
          icon: (viewSettings == null ? void 0 : viewSettings.rtl) ? /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowGoBackLine, null) : /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowGoForwardLine, null),
          onClick: handleGoForward,
          tooltip: _("Go Forward"),
          disabled: !(view == null ? void 0 : view.history.canGoForward)
        }
      ),
      /* @__PURE__ */ React42__namespace.default.createElement(
        Slider_default,
      ...
      /* @__PURE__ */ React42__namespace.default.createElement(
        Button_default,
        {
          icon: (viewSettings == null ? void 0 : viewSettings.rtl) ? /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowLeftWideLine, null) : /* @__PURE__ */ React42__namespace.default.createElement(ri.RiArrowRightWideLine, null),
          onClick: (viewSettings == null ? void 0 : viewSettings.rtl) ? handleGoPrev : handleGoNext,
          tooltip: (viewSettings == null ? void 0 : viewSettings.rtl) ? _("Go Left") : _("Go Right")
        }
      )
    ),
    /* @__PURE__ */ React42__namespace.default.createElement(
      ...
      /* @__PURE__ */ React42__namespace.default.createElement(
        Slider_default,
        {
          initialValue: (viewSettings == null ? void 0 : viewSettings.defaultFontSize) && !isNaN(viewSettings.defaultFontSize) ? viewSettings.defaultFontSize : 16,
          bubbleLabel: `${(viewSettings == null ? void 0 : viewSettings.defaultFontSize) && !isNaN(viewSettings.defaultFontSize) ? viewSettings.defaultFontSize : 16}`,
          minLabel: "A",
          maxLabel: "A",
      ...
      /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "flex w-full items-center justify-between gap-x-6" }, /* @__PURE__ */ React42__namespace.default.createElement(
        Slider_default,
        {
          initialValue: !isNaN(getMarginProgressValue(
            (viewSettings == null ? void 0 : viewSettings.marginPx) ?? 44,
            (viewSettings == null ? void 0 : viewSettings.gapPercent) ?? 5
        ...
        Slider_default,
        {
          initialValue: (viewSettings == null ? void 0 : viewSettings.lineHeight) && !isNaN(viewSettings.lineHeight) ? Math.round(viewSettings.lineHeight * 10) : 16,
    ...
    /* @__PURE__ */ React42__namespace.default.createElement(
      "div",
      {
        className: clsx8__default.default(
          ...
          (appService == null ? void 0 : appService.hasSafeAreaInset) && "pb-[calc(env(safe-area-inset-bottom)+16px)]"
        )
      },
      /* @__PURE__ */ React42__namespace.default.createElement(
        Button_default,
      ...
      /* @__PURE__ */ React42__namespace.default.createElement(
        Button_default,
        {
          icon: /* @__PURE__ */ React42__namespace.default.createElement(
            ...
            {
              size: fontIconSize,
              className: clsx8__default.default(actionTab === "font" && "text-blue-500")
            }
          ),
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      ...
      style: isVertical ? {
        ...
        paddingBottom: (appService == null ? void 0 : appService.hasSafeAreaInset) ? "env(safe-area-inset-bottom)" : 0
      }
    },
...
// src/app/reader/components/settings/FontPanel.jsx
init_constants();
init_misc();
var NumberInput = ({
  className,
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement("div", { className: "dropdown dropdown-top" }, /* @__PURE__ */ React42__namespace.default.createElement(
    ...
    "ul",
    {
      ...
      className: clsx8__default.default(
        ...
        (moreOptions == null ? void 0 : moreOptions.length) ? "" : "inline max-h-80 overflow-y-scroll"
      )
    },
...
var FontFace = ({
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement("div", { className: clsx8__default.default("config-item", className) }, /* @__PURE__ */ React42__namespace.default.createElement("span", { className: "" }, label), /* @__PURE__ */ React42__namespace.default.createElement(
    FontDropDown_default,
    {
      ...
      moreOptions: (moreOptions == null ? void 0 : moreOptions.map((option) => ({ option, label: option }))) ?? [],
      selected,
      onSelect,
...
var FontPanel = ({ bookKey }) => {
  ...
  )), (isCJKEnv() || (view == null ? void 0 : view.language.isCJK)) && /* @__PURE__ */ React42__namespace.default.createElement(
    FontFace,
    {
...
var LayoutPanel = ({ bookKey }) => {
  ...
  React42.useEffect(() => {
    ...
    view == null ? void 0 : view.renderer.setAttribute("max-column-count", maxColumnCount);
    view == null ? void 0 : view.renderer.setAttribute("max-inline-size", `${getMaxInlineSize(viewSettings)}px`);
  }, [maxColumnCount]);
  React42.useEffect(() => {
  ...
  React42.useEffect(() => {
    ...
    view == null ? void 0 : view.renderer.setAttribute("max-block-size", `${maxBlockSize}px`);
  }, [maxBlockSize]);
  React42.useEffect(() => {
    var _a2, _b2;
    ...
    if (view) {
      (_b2 = (_a2 = view.renderer).setStyles) == null ? void 0 : _b2.call(_a2, getStyles(viewSettings));
  ...
  React42.useEffect(() => {
    ...
    (_a2 = getView(bookKey)) == null ? void 0 : _a2.renderer.setAttribute("flow", isScrolledMode ? "scrolled" : "paginated");
    (_b2 = getView(bookKey)) == null ? void 0 : _b2.renderer.setAttribute(
      "max-inline-size",
      `${getMaxInlineSize(viewSettings)}px`
    );
    (_e = (_c = getView(bookKey)) == null ? void 0 : (_d = _c.renderer).setStyles) == null ? void 0 : _e.call(_d, getStyles(viewSettings));
  ...
  const langCode = getBookLangCode((_b = (_a = bookData.bookDoc) == null ? void 0 : _a.metadata) == null ? void 0 : _b.language);
...
// src/app/reader/components/settings/ThemeEditor.jsx
var ThemeEditor = ({ customTheme, onSave, onDelete, onCancel }) => {
  ...
  const [lightTextColor, setLightTextColor] = React42.useState(
    (customTheme == null ? void 0 : customTheme.colors.light.fg) || template.light.fg
  );
  const [lightBackgroundColor, setLightBackgroundColor] = React42.useState(
    (customTheme == null ? void 0 : customTheme.colors.light.bg) || template.light.bg
  );
  const [darkTextColor, setDarkTextColor] = React42.useState(
    (customTheme == null ? void 0 : customTheme.colors.dark.fg) || template.dark.fg
  );
  const [darkBackgroundColor, setDarkBackgroundColor] = React42.useState(
    (customTheme == null ? void 0 : customTheme.colors.dark.bg) || template.dark.bg
  );
  const [themeName, setThemeName] = React42.useState((customTheme == null ? void 0 : customTheme.label) || _("Custom"));
...
// src/app/reader/components/settings/ColorPanel.jsx
var ColorPanel = ({ bookKey }) => {
  const _ = useTranslation();
...
var DialogMenu = ({ toggleDropdown }) => {
  ...
  const handleToggleGlobal = () => {
    ...
    toggleDropdown == null ? void 0 : toggleDropdown();
  };
  return /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
...
// src/app/reader/components/settings/MiscPanel.jsx
var MiscPanel = ({ bookKey }) => {
  ...
  const applyStyles = () => {
    var _a, _b, _c;
    const formattedCSS = cssbeautify__default.default(draftStylesheet, {
      indent: "  ",
    ...
    (_c = (_a = getView(bookKey)) == null ? void 0 : (_b = _a.renderer).setStyles) == null ? void 0 : _c.call(_b, getStyles(viewSettings));
  };
  const handleInput = (e) => {
    e.stopPropagation();
  ...
  const handleInputFocus = () => {
    if (appService == null ? void 0 : appService.isAndroidApp) {
      setInputFocusInAndroid(true);
    }
    setTimeout(() => {
      var _a;
      (_a = textareaRef.current) == null ? void 0 : _a.scrollIntoView({
        behavior: "instant",
        block: "center"
      });
    }, 300);
  ...
  React42.useEffect(() => {
    ...
    if (animated) {
      (_a = getView(bookKey)) == null ? void 0 : _a.renderer.setAttribute("animated", "");
    } else {
      (_b = getView(bookKey)) == null ? void 0 : _b.renderer.removeAttribute("animated");
    }
  }, [animated]);
  React42.useEffect(() => {
...
// src/app/reader/components/settings/SettingsDialog.jsx
var SettingsDialog = ({ bookKey }) => {
  ...
  const tabConfig = [
    ...
    {
      tab: "Color",
      icon: vsc.VscSymbolColor,
      label: _("Color")
    },
    {
      tab: "Misc",
      icon: io5.IoAccessibilityOutline,
      label: _("Misc")
    }
  ];
...
var useNotesSync = (bookKey) => {
  ...
  const getNewNotes = () => {
    if (!(config2 == null ? void 0 : config2.location)) return [];
    const bookNotes = config2.booknotes ?? [];
  ...
  React42.useEffect(() => {
    const processNewNote = (note) => {
      const oldNotes = (config2 == null ? void 0 : config2.booknotes) ?? [];
    ...
    if ((syncedNotes == null ? void 0 : syncedNotes.length) && config2) {
      const newNotes = syncedNotes.filter((note) => note.bookHash === bookHash);
      if (!newNotes.length) return;
      const oldNotes = config2.booknotes ?? [];
      const mergedNotes = [
...
var getPosition = (target, rect, paddingPx2, isVertical = false) => {
  ...
  const [sx, , , sy] = ((_b = (_a = match == null ? void 0 : match[1]) == null ? void 0 : _a.split(/\s*,\s*/)) == null ? void 0 : _b.map((x) => parseFloat(x))) ?? [];
  const frame = (frameElement == null ? void 0 : frameElement.getBoundingClientRect()) ?? { top: 0, left: 0 };
...
// src/app/reader/components/annotator/Annotator.jsx
init_constants();
var Popup = ({
  width,
  height,
  ...
  triangleClassName = "",
  additionalStyle = {}
}) => /* @__PURE__ */ React42__namespace.default.createElement("div", null, /* @__PURE__ */ React42__namespace.default.createElement(
  "div",
  {
    id: "popup-container",
  ...
  "div",
  {
    ...
    style: {
      left: (trianglePosition == null ? void 0 : trianglePosition.dir) === "left" ? `${trianglePosition.point.x}px` : (trianglePosition == null ? void 0 : trianglePosition.dir) === "right" ? `${trianglePosition.point.x}px` : `${trianglePosition ? trianglePosition.point.x : -999}px`,
      top: (trianglePosition == null ? void 0 : trianglePosition.dir) === "up" ? `${trianglePosition.point.y}px` : (trianglePosition == null ? void 0 : trianglePosition.dir) === "down" ? `${trianglePosition.point.y}px` : `${trianglePosition ? trianglePosition.point.y : -999}px`,
      borderLeft: (trianglePosition == null ? void 0 : trianglePosition.dir) === "right" ? "none" : (trianglePosition == null ? void 0 : trianglePosition.dir) === "left" ? `6px solid` : "6px solid transparent",
      borderRight: (trianglePosition == null ? void 0 : trianglePosition.dir) === "left" ? "none" : (trianglePosition == null ? void 0 : trianglePosition.dir) === "right" ? `6px solid` : "6px solid transparent",
      borderTop: (trianglePosition == null ? void 0 : trianglePosition.dir) === "down" ? "none" : (trianglePosition == null ? void 0 : trianglePosition.dir) === "up" ? `6px solid` : "6px solid transparent",
      borderBottom: (trianglePosition == null ? void 0 : trianglePosition.dir) === "up" ? "none" : (trianglePosition == null ? void 0 : trianglePosition.dir) === "down" ? `6px solid` : "6px solid transparent",
      transform: (trianglePosition == null ? void 0 : trianglePosition.dir) === "left" || (trianglePosition == null ? void 0 : trianglePosition.dir) === "right" ? "translateY(-50%)" : "translateX(-50%)"
    }
  }
));
Popup.propTypes = {
  width: PropTypes13__default.default.number.isRequired,
  height: PropTypes13__default.default.number.isRequired,
  position: PropTypes13__default.default.shape({
    point: PropTypes13__default.default.shape({
      x: PropTypes13__default.default.number.isRequired,
      y: PropTypes13__default.default.number.isRequired
    })
  }),
  trianglePosition: PropTypes13__default.default.shape({
    dir: PropTypes13__default.default.oneOf(["left", "right", "up", "down"]),
    point: PropTypes13__default.default.shape({
      x: PropTypes13__default.default.number.isRequired,
      y: PropTypes13__default.default.number.isRequired
    })
  ...
  children: PropTypes13__default.default.node.isRequired,
  className: PropTypes13__default.default.string,
  triangleClassName: PropTypes13__default.default.string,
  additionalStyle: PropTypes13__default.default.object
};
...
// src/app/reader/components/annotator/AnnotationPopup.jsx
...
var WiktionaryPopup = ({
  ...
  React42.useEffect(() => {
    ...
    const langCode = typeof lang === "string" ? lang : lang == null ? void 0 : lang[0];
...
var Annotator = ({ bookKey }) => {
  ...
  const [selection, setSelection] = React42.useState(null);
  ...
  const onLoad = (event) => {
    ...
    const handleSelectionchange = () => {
      if (osPlatform === "ios" || (appService == null ? void 0 : appService.isIOSApp)) return;
    ...
    const handlePointerup = () => {
      const sel = doc.getSelection();
      if (isValidSelection(sel)) {
        if (osPlatform === "ios" || (appService == null ? void 0 : appService.isIOSApp)) {
    ...
    (_a2 = detail.doc) == null ? void 0 : _a2.addEventListener("pointerup", handlePointerup);
    (_b2 = detail.doc) == null ? void 0 : _b2.addEventListener("touchstart", handleTouchstart);
    (_c = detail.doc) == null ? void 0 : _c.addEventListener("touchmove", handleTouchmove);
    (_d = detail.doc) == null ? void 0 : _d.addEventListener("touchend", handleTouchend);
    (_e = detail.doc) == null ? void 0 : _e.addEventListener("selectionchange", handleSelectionchange);
    if (appService == null ? void 0 : appService.isMobile) {
      (_f = detail.doc) == null ? void 0 : _f.addEventListener("contextmenu", (event2) => {
  ...
  const onDrawAnnotation = (event) => {
    ...
    if (style === "highlight") {
      draw(overlayer_js.Overlayer.highlight, { color: hexColor });
      ...
      draw(overlayer_js.Overlayer[style], { writingMode, color: hexColor, padding });
    }
  };
  ...
  const handleDismissPopupAndSelection = () => {
    handleDismissPopup();
    view == null ? void 0 : view.deselect();
    isTextSelected.current = false;
  };
  React42.useEffect(() => {
  ...
  React42.useEffect(() => {
    ...
    const { booknotes = [] } = config2;
    const annotations = booknotes.filter(
      (item) => !item.deletedAt && item.type === "annotation" && item.style && CFI4__namespace.compare(item.cfi, start) >= 0 && CFI4__namespace.compare(item.cfi, end) <= 0
    );
    try {
      Promise.all(annotations.map((annotation) => view == null ? void 0 : view.addAnnotation(annotation)));
    } catch (e) {
      console.error(e);
    }
  }, [progress]);
  const handleCopy = () => {
    ...
    if (selection) (_a2 = navigator.clipboard) == null ? void 0 : _a2.writeText(selection.text);
    const cfi = view == null ? void 0 : view.getCFI(selection.index, selection.range);
    if (!cfi) return;
    const annotation = {
      id: uniqueId(),
  ...
  const handleHighlight = (update = false) => {
    ...
    const views = getViewsById(bookKey.split("-")[0]);
    if (existingIndex !== -1) {
      views.forEach((view2) => view2 == null ? void 0 : view2.addAnnotation(annotation, true));
      if (update) {
        annotation.id = annotations[existingIndex].id;
        annotations[existingIndex] = annotation;
        views.forEach((view2) => view2 == null ? void 0 : view2.addAnnotation(annotation));
      } else {
  ...
  const handleExportMarkdown = (event) => {
    ...
    const config3 = getConfig(bookKey);
    const { booknotes: allNotes = [] } = config3;
    ...
    Object.values(booknoteGroups).forEach((group) => {
      group.booknotes.sort((a2, b) => {
        return CFI4__namespace.compare(a2.cfi, b.cfi);
      });
    });
    ...
    (_a2 = navigator.clipboard) == null ? void 0 : _a2.writeText(markdownContent);
    eventDispatcher.dispatch("toast", {
      type: "info",
    ...
    if (appService == null ? void 0 : appService.isMobile) return;
  ...
  const selectionAnnotated = selection == null ? void 0 : selection.annotated;
  const buttons = [
    { tooltipText: _("Copy"), Icon: fi.FiCopy, onClick: handleCopy },
    {
      tooltipText: selectionAnnotated ? _("Delete Highlight") : _("Highlight"),
      Icon: selectionAnnotated ? ri.RiDeleteBinLine : pi.PiHighlighterFill,
      onClick: handleHighlight
    },
    { tooltipText: _("Annotate"), Icon: bs.BsPencilSquare, onClick: handleAnnotate },
    { tooltipText: _("Search"), Icon: fi.FiSearch, onClick: handleSearch },
    { tooltipText: _("Dictionary"), Icon: tb.TbHexagonLetterD, onClick: handleDictionary },
    { tooltipText: _("Wikipedia"), Icon: fa.FaWikipediaW, onClick: handleWikipedia },
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement("div", null, showWiktionaryPopup && trianglePosition && dictPopupPosition && /* @__PURE__ */ React42__namespace.default.createElement(
    ...
    {
      word: selection == null ? void 0 : selection.text,
      lang: (_a = bookData.bookDoc) == null ? void 0 : _a.metadata.language,
    ...
    {
      text: selection == null ? void 0 : selection.text,
      lang: (_b = bookData.bookDoc) == null ? void 0 : _b.metadata.language,
      position: dictPopupPosition,
      trianglePosition,
      popupWidth: dictPopupWidth,
      popupHeight: dictPopupHeight
...
var FootnotePopup = ({ bookKey, bookDoc }) => {
  ...
  const [trianglePosition, setTrianglePosition] = React42.useState(null);
  const [popupPosition, setPopupPosition] = React42.useState(null);
  ...
  React42.useEffect(() => {
    const handleBeforeRender = (e) => {
      ...
      view2.addEventListener("link", (e2) => {
        ...
        (_a2 = footnoteHandler.handle(bookDoc, e2)) == null ? void 0 : _a2.catch((err) => {
          var _a3;
          console.warn(err);
          (_a3 = getView(bookKey)) == null ? void 0 : _a3.goTo(popupLinkDetail.href);
        });
      });
      footnoteViewRef.current = view2;
      (_a = footnoteRef.current) == null ? void 0 : _a.replaceChildren(view2);
      ...
      (_b = renderer.setStyles) == null ? void 0 : _b.call(renderer, `${mainStyles}
${footnoteStyles}`);
    };
  ...
  const docLinkHandler = async (event) => {
    ...
    setGridRect(rect);
    setTrianglePosition(triangPos);
    (_a = footnoteHandler.handle(bookDoc, event)) == null ? void 0 : _a.catch((err) => {
      console.warn(err);
      const detail2 = event.detail;
      view == null ? void 0 : view.goTo(detail2.href);
    });
  };
  const closePopup = () => {
    var _a;
    const view2 = (_a = footnoteRef.current) == null ? void 0 : _a.querySelector("foliate-view");
    view2 == null ? void 0 : view2.close();
    view2 == null ? void 0 : view2.remove();
  };
  const handleDismissPopup = () => {
    closePopup();
    setGridRect(null);
  ...
  React42.useEffect(() => {
    ...
    if (footnoteViewRef.current) {
      (_a = footnoteRef.current) == null ? void 0 : _a.replaceChildren(footnoteViewRef.current);
    }
  }, [footnoteRef]);
...
var DoubleBorder = ({
  ...
  ), showFooter && /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      className: "borderframe pointer-events-none absolute",
      style: {
  ...
  ), showHeader && /* @__PURE__ */ React42__namespace.default.createElement(
    "div",
    {
      className: "borderframe pointer-events-none absolute",
      style: {
...
// src/app/reader/components/BooksGrid.jsx
var BooksGrid = ({ bookKeys, onCloseBook }) => {
  ...
  const { isFontLayoutSettingsDialogOpen, setFontLayoutSettingsDialogOpen } = useSettingsStore();
  const [gridTemplate, setGridTemplate] = React42.useState({ columns: "1fr", rows: "1fr" });
  React42.useEffect(() => {
    if (typeof window !== "undefined") {
      setGridTemplate(grid_default(bookKeys.length, window.innerWidth / window.innerHeight));
    }
  }, [bookKeys.length]);
  React42.useEffect(() => {
    if (!sideBarBookKey) return;
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement(
    ...
    bookKeys.map((bookKey, index) => {
      ...
      const viewSettings = getViewSettings(bookKey);
      const { book, bookDoc } = bookData || {};
      if (!book || !config2 || !bookDoc || !viewSettings) return null;
      const { section, pageinfo, sectionLabel } = progress || {};
      const isBookmarked = (_a = getViewState(bookKey)) == null ? void 0 : _a.ribbonVisible;
      ...
      return /* @__PURE__ */ React42__namespace.default.createElement(
        "div",
        {
          ...
          className: clsx8__default.default(
            ...
            !isSideBarVisible && (appService == null ? void 0 : appService.hasRoundedWindow) && "rounded-window"
          )
        },
        ...
        /* @__PURE__ */ React42__namespace.default.createElement(FoliateViewer_default, { bookKey, bookDoc, config: config2 }),
        ...
        isFontLayoutSettingsDialogOpen && /* @__PURE__ */ React42__namespace.default.createElement(SettingsDialog_default, { bookKey, config: config2 })
      );
    })
  );
};
...
// src/services/tts/WebSpeechClient.js
...
async function* speakWithBoundary(ssml, getRate, getPitch, getVoice) {
  ...
  utterance.onboundary = (event) => {
    ...
    queue.enqueue({
      ...
      speaking: true,
      name: event.name,
      mark: (mark == null ? void 0 : mark.name) ?? "",
...
var WebSpeechClient = class {
  ...
  async getAllVoices() {
    return this.#voices.filter((voice) => {
      ...
      const isNotBlacklisted = (voice2) => {
        return isValidVoice(voice2.name);
      };
  ...
  getVoiceId() {
    var _a;
    return ((_a = this.#voice) == null ? void 0 : _a.name) || "";
  }
};
...
${content2}`;
    ...
    const getHeadersAndData = (message) => {
      ...
      for (lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        const line = (_a = lines[lineIdx]) == null ? void 0 : _a.trim();
        if (!line) break;
    ...
    const config2 = genSendContent(configHeaders, configContent);
    return new Promise((resolve, reject) => {
  ...
  async create(payload) {
    ...
    const cached = _EdgeSpeechTTS.audioCache.get(hash);
    if (cached) {
      return new Response(cached);
    }
    const response = await this.#fetchEdgeSpeechWs(payload);
    const arrayBuffer = await response.arrayBuffer();
    _EdgeSpeechTTS.audioCache.set(hash, arrayBuffer);
...
// src/services/tts/EdgeTTSClient.js
var EdgeTTSClient = class {
  ...
  async stopInternal() {
    ...
    if (this.#audioElement) {
      ...
      if ((_a = this.#audioElement) == null ? void 0 : _a.onended) {
  ...
  getVoiceId() {
    var _a;
    return ((_a = this.#voice) == null ? void 0 : _a.id) || "";
  }
};

// src/services/tts/TTSController.js
var TTSController = class extends EventTarget {
      ...
      try {
        ...
        for await (const { code, mark } of iter) {
          ...
          if (mark && this.state === "playing") {
            (_a = this.view.tts) == null ? void 0 : _a.setMark(mark);
          }
          lastCode = code;
        }
  ...
  async start() {
    var _a, _b;
    await this.initViewTTS();
    const ssml = this.state.includes("paused") ? (_a = this.view.tts) == null ? void 0 : _a.resume() : (_b = this.view.tts) == null ? void 0 : _b.start();
  ...
  async backward() {
    ...
    if (this.state === "playing") {
      await this.stop();
      this.#speak((_a = this.view.tts) == null ? void 0 : _a.prev());
    } else {
      await this.stop();
      this.#speak((_b = this.view.tts) == null ? void 0 : _b.prev());
    }
  }
  // goto next sentence
  async forward() {
    ...
    if (this.state === "playing") {
      await this.stop();
      this.#speak((_a = this.view.tts) == null ? void 0 : _a.next());
    } else {
      await this.stop();
      this.#speak((_b = this.view.tts) == null ? void 0 : _b.next());
    }
  }
  /**
   * @param {string} lang
...
var TTSPanel = ({
  ...
  const [rate, setRate] = React42.useState((viewSettings == null ? void 0 : viewSettings.ttsRate) ?? 1);
  const [selectedVoice, setSelectedVoice] = React42.useState((viewSettings == null ? void 0 : viewSettings.ttsVoice) ?? "");
...
var TTSControl = () => {
  ...
  const handleTTSSpeak = async (event) => {
    ...
    try {
      if (appService == null ? void 0 : appService.isIOSApp) {
        await invokeUseBackgroundAudio({ enabled: true });
      }
      if (getOSPlatform() === "ios" || (appService == null ? void 0 : appService.isIOSApp)) {
        unblockAudio();
      }
      const ttsController = new TTSController(view);
      await ttsController.init();
      await ttsController.initViewTTS();
      const ssml = (_a = view.tts) == null ? void 0 : _a.from(range);
      if (ssml) {
        const lang = parseSSMLLang(ssml) || "en";
        setTtsLang(lang);
        setIsPlaying(true);
  ...
  const handleStop = async () => {
    ...
    if (ttsController) {
      ...
      ttsControllerRef.current = null;
      (_a = getView(bookKey)) == null ? void 0 : _a.deselect();
      setIsPlaying(false);
      setShowPanel(false);
  ...
  return /* @__PURE__ */ React42__namespace.default.createElement("div", null, showPanel && /* @__PURE__ */ React42__namespace.default.createElement(
    ...
    "div",
    {
      ...
      className: clsx8__default.default(
        ...
        (appService == null ? void 0 : appService.hasSafeAreaInset) ? "bottom-[calc(env(safe-area-inset-bottom)+70px)]" : "bottom-[70px] sm:bottom-14"
      )
    },
...
var ReaderContent = ({ ids }) => {
  ...
  React42.useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      (_a = eventDispatcher) == null ? void 0 : _a.on("quit-app", handleCloseBooks);
      return () => {
        var _a2;
        unlisten();
        window.removeEventListener("beforeunload", handleCloseBooks);
        (_a2 = eventDispatcher) == null ? void 0 : _a2.off("quit-app", handleCloseBooks);
      };
    }
  }, [bookKeys]);
  ...
  const saveConfigAndCloseBook = async (bookKey) => {
    ...
    try {
      (_a = getView(bookKey)) == null ? void 0 : _a.close();
      (_b = getView(bookKey)) == null ? void 0 : _b.remove();
    } catch {
  ...
  if (!bookData || !bookData.book || !bookData.bookDoc) {
    ...
    return loading && /* @__PURE__ */ React42__namespace.createElement("div", { className: clsx8__default.default("hero hero-content", (appService == null ? void 0 : appService.isIOSApp) ? "h-[100vh]" : "h-dvh") }, /* @__PURE__ */ React42__namespace.createElement(Spinner_default, { loading: true }));
  }
  return /* @__PURE__ */ React42__namespace.createElement("div", { className: clsx8__default.default("flex", (appService == null ? void 0 : appService.isIOSApp) ? "h-[100vh]" : "h-dvh") }, /* @__PURE__ */ React42__namespace.createElement(SideBar_default, { onGoToLibrary: handleCloseBooksAndReload }), /* @__PURE__ */ React42__namespace.createElement(BooksGrid_default, { bookKeys, onCloseBook: handleCloseBook }), /* @__PURE__ */ React42__namespace.createElement(TTSControl_default, null), /* @__PURE__ */ React42__namespace.createElement(Notebook_default, null), showDetailsBook && /* @__PURE__ */ React42__namespace.createElement(
...
// src/app/reader/components/Reader.jsx
var Reader = ({ bookUrl: propBookUrl }) => {
  console.log({ propBookUrl });
  const defaultBookUrl = "https://cdn.readest.com/books/this-side-of-paradise.epub";
  const [urlBookUrl, setUrlBookUrl] = React42.useState(null);
  const [urlParamsProcessed, setUrlParamsProcessed] = React42.useState(false);
  ...
  return (settings == null ? void 0 : settings.globalReadSettings) && /* @__PURE__ */ React42__namespace.createElement(
    "div",
    {
      className: clsx8__default.default(
